...
-- サンプル投稿5: 公開済み（PHP vs Lua技術記事）
INSERT INTO posts (title, slug, content, excerpt, author_id, status, published_at, created_at, updated_at)
VALUES
(
    'PHPブログツールの「あれ？」なところ：もっとサクサク動かすには？',
    'php-blog-performance-tips',
    ...
    'PHP製ブログツールの欠点について、小学6年生向けに分かりやすく解説。PHP-FPMのメモリ消費、PHP8とOPcacheによる速度改善、OpenResty + LuaJITとの比較を通じて、ブログをサクサク動かすための技術の裏側を探ります。',
    1,
    'published',
...
-- 投稿5: 'php-blog-performance-tips' -> 'tech'カテゴリー
INSERT INTO post_categories (post_id, category_id)
SELECT p.id, c.id
FROM posts p, categories c
WHERE p.slug = 'php-blog-performance-tips' AND c.slug = 'tech';

-- 投稿5: 'php-blog-performance-tips' -> 'lua', 'openresty'タグ
INSERT INTO post_tags (post_id, tag_id)
SELECT p.id, t.id
FROM posts p, tags t
WHERE p.slug = 'php-blog-performance-tips' AND t.slug = 'lua';

INSERT INTO post_tags (post_id, tag_id)
SELECT p.id, t.id
FROM posts p, tags t
WHERE p.slug = 'php-blog-performance-tips' AND t.slug = 'openresty';

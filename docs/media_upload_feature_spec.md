...
- 生成失敗は非同期リトライ対象としてジョブ化する
  - 指数バックオフ
  - 上限回数超過時は運用アラート対象
- 画像ライブラリが WebP 生成に対応している場合は、サムネイルを WebP 形式へ統一することを優先検討する
  - 原本は元形式のまま保持する
  - サムネイル URL は実際の生成形式に合わせる
  - WebP 非対応環境や変換失敗時は元拡張子のサムネイル生成へフォールバックする
  - アニメーション GIF は静止画化の可否とユーザー期待が異なるため、Phase 1 では元形式維持または別途方針決定とする
...
- 物理削除時は原本とサムネイルの両方を削除対象にする
- 物理削除ジョブは DB 更新とファイル削除の不整合を検知できるよう、削除失敗をログと再試行対象に残す
...
- WebP サムネイル生成を有効化した場合に、対応環境では WebP が生成され、非対応または変換失敗時はフォールバックが機能すること
...
1. [`docs/media_upload_design.md`](docs/media_upload_design.md) と本書の差分を確認し、Phase 1 の必須範囲を確定する
2. 保存先 Volume、静的配信、サイズ上限、許可形式の設定値を確定する
3. 画像処理ライブラリとサムネイル生成方式を選定する
4. 参照中メディアの削除可否と物理削除までの猶予期間を確定する
...
2. WebP サムネイル生成とフォールバック方針を実装または採用可否を確定
3. 論理削除済みファイルの物理削除ジョブを追加
